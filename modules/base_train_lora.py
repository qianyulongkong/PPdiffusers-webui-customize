...
# 设置警告过滤器，将 DeprecationWarning 警告静默
warnings.filterwarnings("ignore", category=DeprecationWarning)


import paddlenlp
paddlenlp.utils.log.logger.set_level("ERROR")
from helper.lora_helper import extract_lora_weights,convert_pd_to_pt
import itertools
import argparse
import contextlib
import gc
...
def write_jsonl(base_dir, image_format):
    from pathlib import Path
    import json
    import os
    with open(os.path.join(base_dir, "metadata.jsonl"), "w", encoding="utf-8" ) as json_file:
        for p in Path(base_dir).glob("*.txt"):
            jsonline = {"file_name": "0001.png", "text": "This is a golden retriever playing with a ball"}
            with open(p, "r") as f:
                line = f.readlines()[0].strip()
            jsonline["file_name"] = p.name.replace("txt", image_format)
            jsonline["text"] = line
            json.dump(jsonline, json_file)
            json_file.write("\n")
...
def main():
    ...
    if args.debug:
        ...
        if args.dataset_name is not None:
            ...
            write_jsonl(args.train_data_dir, args.image_format)
            dataset = load_dataset(
                "imagefolder",
    ...
    for epoch in range(args.num_train_epochs):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        tmp_dict = {}
                        tmp_dict.update(extract_lora_weights(text_encoder))
                        tmp_dict.update(extract_lora_weights(unet))
                        paddle.save(tmp_dict, os.path.join(save_path, "text_encoder_unet_lora.pdparams"))
                        save_file(
                            convert_pd_to_pt(tmp_dict),
        ...
        if is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                images = [
                    pipeline(args.validation_prompt, num_inference_steps=30, generator=generator, guidance_scale=args.validation_guidance_scale).images[0]
    ...
    tmp_dict.update(extract_lora_weights(unet))
    paddle.save(tmp_dict, os.path.join(save_path, "text_encoder_unet_lora.pdparams"))
    save_file(convert_pd_to_pt(tmp_dict), os.path.join(save_path, "text_encoder_unet_lora.safetensors"))
